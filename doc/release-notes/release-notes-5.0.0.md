TrumpCoin Core version *v5.0.0* is now available from:  <https://github.com/trumpcoin-project/trumpcoin/releases>
...
TrumpCoin Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
TrumpCoin Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.10+, and Windows 7 and later.

Microsoft ended support for Windows XP on [April 8th, 2014](https://www.microsoft.com/en-us/WindowsForBusiness/end-of-xp-support), No attempt is made to prevent installing or running the software on Windows XP, you can still do so at your own risk but be aware that there are known instabilities and issues. Please do not report issues about Windows XP to the issue tracker.

Apple released it's last Mountain Lion update August 13, 2015, and officially ended support on [December 14, 2015](http://news.fnal.gov/2015/10/mac-os-x-mountain-lion-10-8-end-of-life-december-14/). TrumpCoin Core software starting with v3.2.0 will no longer run on MacOS versions prior to Yosemite (10.10). Please do not report issues about MacOS versions prior to Yosemite to the issue tracker.
...
#### Overview
Users will be able to protect their financial information by sending and receiving TRUMPs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://trumpcoin.org .
...
* Top bar showing shield TRUMP balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield TRUMP balance.
...
See the release notes from [v4.2.0](https://github.com/TrumpCoin-Project/TrumpCoin/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed TRUMP supply information | No |
| `initpatriotnode` | Manually initialize the client as a patriotnode | No |
| `getcachedblockhashes` | Return the block hashes cached in the patriotnode manager | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in TRUMP
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in TRUMP
...
* `initpatriotnode`
  ...
  Arguments:
  1. patriotnodePrivKey          (string, required) The patriotnode private key.
  2. patriotnodeAddr             (string, required) The IP:Port of this patriotnode.
...
* `startpatriotnode`
  A new optional input argument (`reload_conf`) has been added to reload the client's `patriotnode.conf` file in real-time. This optional argument is only available when running `startpatriotnode` with "alias" being specified as the first argument.
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zTRUMPsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield TRUMP)
  The `moneysupply` JSON return entry is the total TRUMP supply (the sum of all transparent and shield TRUMP)
...
| `getaccountaddress` | Returned the current TrumpCoin address associated with an internal account | Account system removal |
| `setaccount` | Associate a TrumpCoin address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given TrumpCoin address | Account system removal |
| `getaddressesbyaccount` | Returned a list of TrumpCoin addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a TrumpCoin address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a TrumpCoin address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zTRUMP balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zTRUMP mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zTRUMP mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zTRUMP amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zTRUMP to a TRUMP address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zTRUMP mints to a TRUMP address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zTRUMPs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzpivseed` | Set the wallet's deterministic zTRUMP seed to a specific value | Zerocoin deprecation |
| `getzpivseed` | Return the current deterministic zTRUMP seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zTRUMP seed | Zerocoin deprecation |
| `dzpivstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzpiv` | Do a search for deterministically generated zTRUMP that have not yet added to the wallet | Zerocoin deprecation |
...
- #2030 `f3a753d4c8` [GUI] Workaround to the PN MISSING/REMOVE labeling (furszy)
...
- #1868 `fb94ca77d4` [wallet] GetPatriotnodeVinAndKeys wasn't locking cs_wallet to obtain the collateral. (furszy)
...
- #1930 `2dc9704ba2` [RPC] Speeding up listpatriotnodes command (furszy)
...
### Patriotnodes/Budget
- #1829 `0adce3cbb7` [Patriotnodes] Tier two network sync new architecture, regtest support + PN activation functional test. (furszy)
- #1834 `9d5f5952e0` [Patriotnodes] CActivePatriotnode::ManageStatus remove unneded GetAvailableBalance check (furszy)
...
- #1861 `8f37bcccc3` [Refactoring] Budget, round 6: patriotnode vote for finalized budgets (random-zebra)
- #1862 `70c1fd1981` [Refactoring] Budget, round 7: split patriotnode-budget files (random-zebra)
- #1886 `db9b0933d5` [Patriotnodes] dead end over the activation process. (furszy)
...
- #2006 `9ce3fec43c` [Refactor] Patriotnode activeState on-demand (random-zebra)
...
- #2031 `e7fcab0019` [Patriotnodes] remove recursive lock in CPatriotnode::Status() (random-zebra)
- #2045 `1617f9d347` [TierTwo] Remove unneeded extra logging. (furszy)
- #2056 `945044c740` [Patriotnode] Encapsulate global and do not parse privkey redundantly everywhere. (furszy)
- #2060 `c246834912` [Bug][PN] Immediately detect forked patriotnodes (random-zebra)
- #2068 `6db9e9c189` [Consensus] Budget: Fix instant killing of proposals (random-zebra)
- #2084 `11606ee0d4` [BUG][Patriotnode] Miner not paying valid finalized budget if <20 active patriotnodes (random-zebra)
- #2103 `7a1837eb58` [Startup] Do not block local peer port selection for Patriotnodes running over tor. (furszy)
...
- #1904 `92035f80fe` [Validation] Introduce rolling cache for block hashes in patriotnode manager (random-zebra)
...
- #2000 `0756462149` [Core] Patriotnode collateral index (furszy)
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_trumpcoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake trumpcoind and unit tests: fixing missing MacOS framework. (furszy)
...
- #1922 `2d86a41951` [Refactor] Strip out cs_main lock for misbehaving from Patriotnodeman:ProcessMessage (furszy)
