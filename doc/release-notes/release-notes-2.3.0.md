TrumpCoin Core version 2.3.0 is now available from:
...
TrumpCoin Core v2.3.0 is a mandatory update for all users. This release contains validation and security improvements that are not backwards compatible with older versions. Users will have a grace period of no less than two weeks to update their clients before enforcement of this update is enabled.
...
TrumpCoin Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.8+, and Windows Vista and later.
...
#### patriotnode command
The `patriotnode` RPC command has been re-worked to ease it's usage and return valid JSON in it's results. The following is an overview of the changed command parameters:
...
| `list` | Remove all optional "modes" and standardized the results. Note: `patriotnode list` is the same as `patriotnodelist`. See below |
...
[
  {
    ...
    winner: {
        address: addr,    (string) TrumpCoin PN Address,
...
For the `list` (aka `patriotnodelist`) parameter, the various "modes" have been removed in favor of a unified and standardized result format. The result is now an array of objects instead of an object of objects. Further, the individual objects now have a standard JSON format. The result format is as follows:
...
[
  {
    ...
    "lastseen": ttt,   (numeric) The time in seconds since epoch (Jan 1 1970 GMT) the patriotnode was last seen
...
An additional parameter has been added to `mnbudget` to allow a controller wallet to issue per-PN votes. The new parameter is `vote-alias` and it's use format is as follows:
...
trumpcoind can now (optionally) asynchronously notify clients through a ZMQ-based PUB socket of the arrival of new transactions and blocks. This feature requires installation of the ZMQ C API library 4.x and configuring its use through the command line or configuration file. Please see [docs/zmq.md](/doc/zmq.md) for details of operation.

**All** Patriotnodes List GUI Removal
--------------

With the standardization and reformatting of the `patriotnode list` (`patriotnodelist`) RPC command, there is no real use case to keep the full list of patriotnodes in the GUI. This GUI element causes a great deal of extra overhead, even when it is not being actively displayed. The removal of this list has also proven to resolve a number of linux-based errors

Note that the GUI list of patriotnodes associated with a controller wallet remains intact.

SPV Client Support
--------------

TrumpCoin Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for TrumpCoin (Mrs-X)
- #211 `b8c110b` [RPC] Refactor & JSONify results from patriotnode command(s) (Fuzzbawls)
...
- #234 `ed99e7b` [Consensus/Net] Ignore newly activated PNs in ranking/seesaw (Mrs-X Fuzzbawls presstab)

### GUI
- #200 `bb1f255` [UI] Improved unlock usability (Mrs-X)
- #207 `7a41f46` [Qt] Adjust size of splash screen image. (Fuzzbawls)
- #206 `9c675ee` [Qt] Remove the All Patriotnodes UI tab/list (Fuzzbawls)
- #220 `b80bc29` [Qt] Add "NODE_BLOOM" and "NODE_BLOOM_WITHOUT_PN" to guiutil (Fuzzbawls)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with TrumpCoin (Fuzzbawls)
...
- #213 `ddd8994` [Trivial] Reduce debug.log spam for patriotnode messages (Fuzzbawls)
