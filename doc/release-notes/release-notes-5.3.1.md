TrumpCoin Core version v5.3.1 is now available from: https://github.com/trumpcoin-project/trumpcoin/releases

This is an important release for stakers and Patriotnodes: including several tier two synchronization, GUI and build system bug fixes.
...
If you are running an older version, shut it down. Wait until it has completely shut down, then run the installer (on Windows) or just copy over /Applications/TrumpCoin-Qt (on Mac) or trumpcoind/trumpcoin-qt (on Linux).
...
### GUI
- #2530 [GUI][BUG] remove duplicated coll. confirmation check in PNModel::data (random-zebra)
