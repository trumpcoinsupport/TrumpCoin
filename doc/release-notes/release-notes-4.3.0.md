TrumpCoin Core version *4.3.0* is now available from:  <https://github.com/trumpcoin-project/trumpcoin/releases>
...
- The chainstate database (which is used for tracking UTXOs) has been changed from a per-transaction model to a per-output model ([See PR 1801](https://github.com/TrumpCoin-Project/TrumpCoin/pull/1801)). Advantages of this model are that it:
...
- LevelDB has been upgraded to version 1.22 ([See PR 1738](https://github.com/TrumpCoin-Project/TrumpCoin/pull/1738)). This version contains hardware acceleration for CRC on architectures supporting SSE 4.2. As a result, synchronization and block validation are now faster.
...
`mnping` related debug log messages have been moved to their own category of the same name. This is to reduce log spam when debugging with the `patriotnode` category enabled.
...
### GUI
 ...
 - #1782 `ada4462782` [GUI] Start patriotnodes polling timer only when is needed. (furszy)
...
### Layer 2 (PN/Budget)
 - #1791 `6162df962b` [Patriotnodes] Missing cs main locks in CalculateScore and GetPatriotnodeInputAge (furszy)
 - #1803 `69ec4a3fdf` [Cleanup] patriotnode-budget tiny cleanup. (furszy)
 - #1825 `a06c0fd993` [PN] more cleanup over the tier two area. (furszy)
 - #1826 `961f5373bf` [Refactor] Patriotnode Budget first refactoring and cleanup (random-zebra)
...
### Miner/Block Generation
 ...
 - #1809 `959d707bc9` [Miner] decouple zTRUMP duplicated serials checks from CreateNewBlock (furszy)
...
### Miscellaneous
 ...
 - #1821 `4b3fb02dc3` [Cleanup] removing unused GetPatriotnodeByRank method (furszy)
 - #1822 `48d7475bd4` [Refactor] Dedicated logging category for patriotnode pings (random-zebra)
 ...
 - #1833 `9c06e5d7ce` [Refactor] Remove GetInputAge and GetPatriotnodeInputAge (random-zebra)
