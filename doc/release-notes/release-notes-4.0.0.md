TrumpCoin Core version *v4.0.0* is now available from:  <https://github.com/trumpcoin-project/trumpcoin/releases>
...
TrumpCoin Core v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
You can read more details about this extensive work in ([PR #954](https://github.com/TrumpCoin-Project/TrumpCoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/TrumpCoin-Project/TrumpCoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [TrumpCoin Wiki](https://github.com/TrumpCoin-Project/TrumpCoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/TrumpCoin-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/TrumpCoin-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/TrumpCoin-Project/TrumpCoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 TRUMP, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zTRUMP Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zTRUMP have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zTRUMP with public spends version 4 ([PR #936](https://github.com/TrumpCoin-Project/TrumpCoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zTRUMP), version 4 spends will also be public. A full technical writeup is available on the [TrumpCoin Wiki](https://github.com/TrumpCoin-Project/TrumpCoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/TrumpCoin-Project/TrumpCoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (PN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/TrumpCoin-Project/TrumpCoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/TrumpCoin-Project/TrumpCoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/TrumpCoin-Project/TrumpCoin/pull/995)).<br>
...
- `delegated_balance` - TRUMP balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - TRUMP balance held in cold staking addresses.
...
Arguments:
1. "stakingaddress"      (string, required) The trumpcoin staking address to delegate.
2. "amount"              (numeric, required) The amount in TRUMP to delegate for staking. eg 100
3. "owneraddress"        (string, optional) The trumpcoin address corresponding to the key that will be able to spend the stake.
...
Result:
amount              (numeric) The total amount in TRUMP received for this account in P2CS contracts.
```
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~trumpcoin) repository, we are now offering a [Snap package](https://snapcraft.io/trumpcoin-core) as quick way to install and update a TrumpCoin wallet.
...
The wallet's internal miner/staker is no longer prevented from running prior to having synced all the additional layer 2 (PN/Budget) data. Instead, mining/staking uses better logic to allow block creation without fully synced layer 2 data when the full data set wouldn't be required.
...
- #936 `12a6b704b6` [zTRUMP] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1050 `adc74f737c` [Core] Prevent coinstakes from overpaying patriotnodes (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for TrumpCoin v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for TrumpCoin v4.0 (random-zebra)
- #1197 `ad241150e9` [Trivial] Update copyright headers (Fuzzbawls)

### GUI
- #954 `e815815fdc` [GUI] [Model] New Wallet UI (furszy)
- #997 `4d50ff33da` [Trivial][UI] PN screen (furszy)
- #998 `6380da7eb4` [Qt] Guard PN tab from possible missing TXs in patriotnode.conf (Fuzzbawls)
...
- #1055 `232cea5584` [Wallet] Create label for addresses generated via patriotnode wizard (CryptoDev-Project)
...
- #1125 `c6e238ca4d` [GUI] Inform if open trumpcoin.conf and/or backups folder fail. (furszy)
...
- #1158 `7deae859f1` [BUG] Patriotnodes wizard (furszy)
...
- #1166 `4cc04b0ba6` [Qt] properly copy IPv6 externalip PN info (Fuzzbawls)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong TrumpCoin website (NoobieDev12)
...
- #1001 `59f55d6a54` [Consensus] Remove Old message format in CPatriotnodeBroadcast (random-zebra)
- #1024 `98aa3fa438` [Consensus] New signatures for network messages (random-zebra)
- #1110 `f89f672847` [Masterndoes] Patriotnodes sync try locking cs_main when it looks for the tip (furszy)
