...
- [Development guidelines](#development-guidelines)
    - [General TrumpCoin Core](#general-trumpcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
TrumpCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play TRUMPs (tTRUMP)" on the test network, if you
...
TrumpCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/trumpcoin-project/trumpcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_trumpcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_trumpcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/trumpcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_trumpcoin.coverage/index.html`.
```

**Sanitizers**
TrumpCoin can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates TRUMPs (if wallet is enabled).
...
However, in open source software such as TrumpCoin Core, where everyone uses
...
pay attention to for reviewers of TrumpCoin Core code.

General TrumpCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: TrumpCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against TrumpCoin Core so that they can be integrated
...
file descriptors it will cause problems with TrumpCoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:TrumpCoin-Project/TrumpCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `trumpcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `trumpcoin-cli` and the GUI debug console use this table to determine how to
