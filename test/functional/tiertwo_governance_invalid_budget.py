...
class GovernanceInvalidBudgetTest(TrumpCoinTestFramework):
    ...
    def run_test(self):
        ...
        self.minerA.mnbudgetvote("alias", proposalHash, "yes", self.patriotnodeOneAlias)
        self.minerA.mnbudgetvote("alias", proposalHash, "yes", self.patriotnodeTwoAlias)
        time.sleep(1)
    ...
    def setupContext(self):
        ...
        self.mnOneCollateral = self.setupPatriotnode(self.minerA, self.minerA, self.patriotnodeOneAlias,
                                                    ownerdir, self.remoteOnePos, self.mnOnePrivkey)
        self.mnTwoCollateral = self.setupPatriotnode(self.minerA, self.minerA, self.patriotnodeTwoAlias,
                                                    ownerdir, self.remoteTwoPos, self.mnTwoPrivkey)

        # Activate patriotnodes
        self.log.info("Patriotnodes activation...")
        ...
        self.mn1.initpatriotnode(self.mnOnePrivkey, "127.0.0.1:"+str(remoteOnePort))
        self.mn2.initpatriotnode(self.mnTwoPrivkey, "127.0.0.1:"+str(remoteTwoPort))
        self.stake_and_ping(self.minerAPos, 1, [])
        self.wait_until_mnsync_finished()
        self.controller_start_patriotnode(self.minerA, self.patriotnodeOneAlias)
        self.controller_start_patriotnode(self.minerA, self.patriotnodeTwoAlias)
        ...
        # activate sporks
        self.log.info("Patriotnodes enabled. Activating sporks.")
        self.activate_spork(self.minerAPos, "SPORK_8_PATRIOTNODE_PAYMENT_ENFORCEMENT")
        self.activate_spork(self.minerAPos, "SPORK_9_PATRIOTNODE_BUDGET_ENFORCEMENT")
