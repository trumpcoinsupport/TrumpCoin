...
"""Test trumpcoind with different proxy configuration.

Test plan:
- Start trumpcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on trumpcoind side:
...
- Create trumpcoinds that connect to them
- Manipulate the trumpcoinds using addnode (onetry) an observe effects
...
class ProxyTest(TrumpCoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: trumpcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
