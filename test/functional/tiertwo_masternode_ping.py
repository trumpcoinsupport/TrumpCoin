...
"""
Test checking patriotnode ping thread
Does not use functions of TrumpCoinTier2TestFramework as we don't want to send
pings on demand. Here, instead, mocktime is disabled, and we just wait with
time.sleep to verify that patriotnodes send pings correctly.
"""
...
class PatriotnodePingTest(TrumpCoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create collateral
        self.log.info("funding patriotnode controller...")
        patriotnodeAlias = "mnode"
        mnAddress = owner.getnewaddress(patriotnodeAlias)
        ...
        confData = patriotnodeAlias + " 127.0.0.1:" + str(p2p_port(2)) + " " + \
                   str(mnPrivkey) +  " " + str(collateralTxId) + " " + str(vout)
        destPath = os.path.join(self.options.tmpdir, "node1", "regtest", "patriotnode.conf")
        with open(destPath, "a+", encoding="utf8") as file_object:
            file_object.write("\n")
            file_object.write(confData)

        # Init remote
        self.log.info("initializing remote patriotnode...")
        remote.initpatriotnode(mnPrivkey, "127.0.0.1:" + str(p2p_port(2)))
        ...
        # Send Start message
        self.log.info("sending patriotnode broadcast...")
        self.controller_start_patriotnode(owner, patriotnodeAlias)
        miner.generate(1)
        self.sync_blocks()
        time.sleep(1)

        # Wait until patriotnode is enabled everywhere (max 180 secs)
        self.log.info("waiting till patriotnode gets enabled...")
        start_time = time.time()
        time.sleep(5)
        self.wait_until_mn_enabled(collateralTxId, 180)
        self.log.info("Patriotnode enabled in %d seconds" % (time.time() - start_time))
        self.log.info("Good. Patriotnode enabled")
        miner.generate(1)
        self.sync_blocks()
        time.sleep(1)
