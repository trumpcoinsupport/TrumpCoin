...
UniValue mnbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a TrumpCoin initiative\n"
            "  vote-alias         - Vote on a TrumpCoin initiative\n"
            "  vote               - Vote on a TrumpCoin initiative/budget\n"
            "  getvotes           - Show current patriotnode budgets\n"
            "  getinfo            - Show current patriotnode budgets\n"
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"trumpcoin-address\" monthy-payment\n"
            ...
            "5. \"trumpcoin-address\":   (string, required) TrumpCoin address to send payments to\n"
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 TRUMP collateral for proposal
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"trumpcoin-address\" monthy-payment \"fee-tx\"\n"
    ...
    if (!patriotnodeSync.IsBlockchainSynced()) {
        throw runtime_error("Must wait for client to sync with patriotnode network. Try again in a minute or so.");
    }
    ...
    budget.mapSeenPatriotnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
...
UniValue mnbudgetvote(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (params.size() == 4 && strCommand != "alias") ||
        ...
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'local' for voting directly from a patriotnode, 'many' for voting with a MN controller and casting the same vote for each MN, 'alias' for voting with a MN controller and casting a vote for a single MN\n"
    ...
    if (strCommand == "local") {
        ...
        while (true) {
            if (!obfuScationSigner.SetKey(strPatriotNodePrivKey, errorMessage, keyPatriotnode, pubKeyPatriotnode)) {
                ...
                statusObj.push_back(Pair("error", "Patriotnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                ...
                statusObj.push_back(Pair("node", "local"));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Failure to find patriotnode in list : " + activePatriotnode.vin.ToString()));
                resultsObj.push_back(statusObj);
                break;
            }

            CBudgetVote vote(activePatriotnode.vin, hash, nVote);
            ...
            if (budget.UpdateProposal(vote, NULL, strError)) {
                success++;
                budget.mapSeenPatriotnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                statusObj.push_back(Pair("node", "local"));
    ...
    if (strCommand == "many") {
        BOOST_FOREACH (CPatriotnodeConfig::CPatriotnodeEntry mne, patriotnodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyPatriotnode, pubKeyPatriotnode)) {
                failed++;
            ...
            if (pmn == NULL) {
                ...
                statusObj.push_back(Pair("node", mne.getAlias()));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Can't find patriotnode by pubkey"));
                resultsObj.push_back(statusObj);
                continue;
            }
...
UniValue getbudgetvotes(const UniValue& params, bool fHelp)
{
    if (params.size() != 1)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"mnId\": \"xxxx\",        (string) Hash of the patriotnode's collateral transaction\n"
...
UniValue mnbudgetrawvote(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "mnbudgetrawvote \"patriotnode-tx-hash\" patriotnode-tx-index \"proposal-hash\" yes|no time \"vote-sig\"\n"
            "\nCompile and relay a proposal vote with provided external signature instead of signing vote internally\n"

            "\nArguments:\n"
            "1. \"patriotnode-tx-hash\"  (string, required) Transaction hash for the patriotnode\n"
            "2. patriotnode-tx-index   (numeric, required) Output index for the patriotnode\n"
...
UniValue mnfinalbudget(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "vote-many") {
        ...
        BOOST_FOREACH (CPatriotnodeConfig::CPatriotnodeEntry mne, patriotnodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyPatriotnode, pubKeyPatriotnode)) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Patriotnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Can't find patriotnode by pubkey"));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if (strCommand == "vote") {
        ...
        CPatriotnode* pmn = mnodeman.Find(activePatriotnode.vin);
        if (pmn == NULL) {
            return "Failure to find patriotnode in list : " + activePatriotnode.vin.ToString();
        }

        CFinalizedBudgetVote vote(activePatriotnode.vin, hash);
        if (!vote.Sign(keyPatriotnode, pubKeyPatriotnode)) {
