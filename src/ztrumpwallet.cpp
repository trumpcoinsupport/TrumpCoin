...
CzTRUMPWallet::CzTRUMPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztrump seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTRUMPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTRUMPSeed_deprecated()) {
                    LogPrintf("%s: Updated zTRUMP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztrump seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztrump wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTRUMPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTRUMPCount(nCountLastUsed);
    else if (!walletdb.ReadZTRUMPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTRUMPWallet::Lock()
{
    seedMaster = 0;
}

void CzTRUMPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTRUMPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTRUMP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTRUMPWallet::LoadMintPoolFromDB()
{
...
void CzTRUMPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTRUMPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTRUMPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztrumpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTRUMPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTRUMP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztrumpTracker which also adds to database
    pwalletMain->ztrumpTracker->Add(dMint, true);
...
void CzTRUMPWallet::SeedToZTRUMP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTRUMPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTRUMPWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZTRUMPCount(nCountLastUsed);
...
void CzTRUMPWallet::GenerateDeterministicZTRUMP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTRUMPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTRUMPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
