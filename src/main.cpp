...
/** Fees smaller than this (in utrump) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zTRUMP is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zTRUMP spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zTRUMP without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zTRUMP spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zTRUMP spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTRUMP spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ztrump");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zTRUMP mints are not already known
...
int64_t GetPatriotnodePayment(int nHeight, int64_t blockValue, int nPatriotnodeCount, bool isZTRUMPStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TrumpCoin
...
{
    RenameThread("trumpcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTRUMPMinted()
{
...
void RecalculateZTRUMPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTRUMP supply
...
bool RecalculateTRUMPSupply(int nHeightStart)
{
...
bool UpdateZTRUMPSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTRUMPSupply(Params().Zerocoin_StartHeight());
    }

    //Track zTRUMP money supply in the block index
    if (!UpdateZTRUMPSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTRUMP supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TRUMP to zTRUMP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("CheckBlock() : Couldn't find patriotnode/budget payment"),
                        REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            if (fDebug)
                LogPrintf("CheckBlock(): Patriotnode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTRUMP spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTRUMP serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zTRUMP stake block is less than allowed start height", __func__);

    if (CZPivStake* zTRUMP = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zTRUMP->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zTRUMP stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zTRUMP stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zTRUMP->GetValue()));
        if (nChecksum200 != zTRUMP->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zTRUMP->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZTRUMP() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zTRUMP fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTRUMP mints and %d zTRUMP spends\n", __func__, nMints, nSpends);
    ...
    if (!fLiteMode) {
        if (patriotnodeSync.RequestedPatriotnodeAssets > PATRIOTNODE_SYNC_LIST) {
            ...
            patriotnodePayments.ProcessBlock(GetHeight() + 10);
            budget.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_BUDGET_VOTE:
        if (budget.mapSeenPatriotnodeBudgetVotes.count(inv.hash)) {
            patriotnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_PROPOSAL:
        if (budget.mapSeenPatriotnodeBudgetProposals.count(inv.hash)) {
            patriotnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_PATRIOTNODE_WINNER) {
                    if (patriotnodePayments.mapPatriotnodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenPatriotnodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenPatriotnodeBudgetVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenPatriotnodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenPatriotnodeBudgetProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_PATRIOTNODE_ANNOUNCE) {
                    if (mnodeman.mapSeenPatriotnodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenPatriotnodeBroadcast[inv.hash];
                        pfrom->PushMessage("mnb", ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_PATRIOTNODE_PING) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        //patriotnode signed transaction
        ...
        if (strCommand == "tx") {
            ...
            //these allow patriotnodes to publish a limited amount of free transactions
            vRecv >> tx >> vin >> vchSig >> sigTime;

            CPatriotnode* pmn = mnodeman.Find(vin);
            if (pmn != NULL) {
                if (!pmn->allowFreeTx) {
                    //multiple peers can send us a valid patriotnode transaction
                    if (fDebug) LogPrintf("dstx: Patriotnode sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubKeyPatriotnode, vchSig, strMessage, errorMessage)) {
                    LogPrintf("dstx: Got bad patriotnode address signature %s \n", vin.ToString());
                    //pfrom->Misbehaving(20);
                    return false;
                }

                LogPrintf("dstx: Got Patriotnode transaction %s\n", tx.GetHash().ToString());
