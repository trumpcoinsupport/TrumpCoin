...
static mnKeyList getPNKeysForActivePatriotnode(UniValue& resultsObj)
{
    ...
    if (!fPatriotNode)
        throw JSONRPCError(RPC_MISC_ERROR, _("This is not a patriotnode. 'local' option disabled."));
...
static mnKeyList getDPNKeys(CWallet* const pwallet, const Optional<std::string>& mnAliasFilter, bool fFinal, UniValue& resultsObj, int& failed)
{
    if (!pwallet) {
...
static mnKeyList getDPNKeysForActivePatriotnode(UniValue& resultsObj)
{
    ...
    if (!activePatriotnodeManager)
        throw JSONRPCError(RPC_MISC_ERROR, _("This is not a deterministic patriotnode. 'local' option disabled."));

    CKey dmnKey; CKeyID dmnKeyID; CDeterministicPNCPtr dmn;
    auto res = activePatriotnodeManager->GetOperatorKey(dmnKey, dmnKeyID, dmn);
    if (!res) {
...
static UniValue mnBudgetVoteInner(CWallet* const pwallet, bool fLegacyPN, const uint256& budgetHash, bool fFinal,
                                  const CBudgetVote::VoteDirection& nVote, const Optional<std::string>& mnAliasFilter)
{
    ...
    mnKeyList mnKeys = fLegacyPN ? getPNKeys(mnAliasFilter, resultsObj, failed)
                                 : getDPNKeys(pwallet, mnAliasFilter, fFinal, resultsObj, failed);
...
static UniValue mnLocalBudgetVoteInner(bool fLegacyPN, const uint256& budgetHash, bool fFinal,
...
UniValue mnbudgetvote(const JSONRPCRequest& request)
{
    ...
    if (!fLegacyPN && !EnsureWalletIsAvailable(pwallet, false)) {
        return NullUniValue;
    }
