...
// A helper object for signing messages from Patriotnodes
...
// Keep track of the used Patriotnodes
std::vector<CTxIn> vecPatriotnodesUsed;
...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_REJECTED, errorID);
        ...
        if (!fPatriotNode) {
            ...
            LogPrintf("dsa -- not a Patriotnode! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_REJECTED, errorID);
        ...
        if (!IsCompatibleWithSession(nDenom, txCollateral, errorID)) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_ACCEPTED, errorID);
            return;
        }
        ...
        CPatriotnode* pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        // if the queue is ready, submit if we can
        if (dsq.ready) {
            if (!pSubmittedToPatriotnode) return;
            if ((CNetAddr)pSubmittedToPatriotnode->addr != (CNetAddr)addr) {
                LogPrintf("dsq - message doesn't match current Patriotnode - %s != %s\n", pSubmittedToPatriotnode->addr.ToString(), addr.ToString());
                return;
            }
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Patriotnode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
        ...
        if (!fPatriotNode) {
            LogPrintf("dsi -- not a Patriotnode! \n");
            errorID = ERR_NOT_A_MN;
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_REJECTED, errorID);
        ...
        if (AddEntry(in, nAmount, txCollateral, out, errorID)) {
            ...
            RelayStatus(sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_RESET);
        } else {
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_REJECTED, errorID);
        }
        ...
        if (!pSubmittedToPatriotnode) return;
        if ((CNetAddr)pSubmittedToPatriotnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dssu - message doesn't match current Patriotnode - %s != %s\n", pSubmittedToPatriotnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (success) {
            ...
            RelayStatus(obfuScationPool.sessionID, obfuScationPool.GetState(), obfuScationPool.GetEntriesCount(), PATRIOTNODE_RESET);
        }
        ...
        if (!pSubmittedToPatriotnode) return;
        if ((CNetAddr)pSubmittedToPatriotnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dsc - message doesn't match current Patriotnode - %s != %s\n", pSubmittedToPatriotnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing || !patriotnodeSync.IsBlockchainSynced()) {
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            if (showingObfuScationMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to patriotnode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                suffix = "...";
            return strprintf(_("Submitted to patriotnode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
        }
    ...
    case POOL_STATUS_QUEUE:
        if (showingObfuScationMessage % 70 <= 30)
            ...
            suffix = "...";
        return strprintf(_("Submitted to patriotnode, waiting in queue %s"), suffix);
        ;
    default:
...
//
// Check the Obfuscation progress and send client updates if a Patriotnode
//
void CObfuscationPool::Check()
{
    if (fPatriotNode) LogPrint("obfuscation", "CObfuscationPool::Check() - entries count %lu\n", entries.size());
    ...
    if (fPatriotNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        ...
        UnlockCoins();
        SetNull();
        if (fPatriotNode) RelayStatus(sessionID, GetState(), GetEntriesCount(), PATRIOTNODE_RESET);
    }
}

void CObfuscationPool::CheckFinalTransaction()
{
    if (!fPatriotNode) return; // check and relay final tx only on patriotnode
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if (!obfuScationSigner.SetKey(strPatriotNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CObfuscationPool::Check() - ERROR: Invalid Patriotnodeprivkey: '%s'\n", strError);
            return;
        }
        ...
        if (!mapObfuscationBroadcastTxes.count(txNew.GetHash())) {
            ...
            dstx.tx = txNew;
            dstx.vin = activePatriotnode.vin;
            dstx.vchSig = vchSig;
...
//
// How does this work? Messages to Patriotnodes come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Patriotnode
    ...
    if (fPatriotNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat TrumpCoin and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 TRUMP per transaction on average.
            */
            if (r <= 10) {
...
{
    if (!fEnableZeromint && !fPatriotNode) return;
    ...
    int addLagTime = 0;
    if (!fPatriotNode) addLagTime = 10000; //if we're the client, give the server a few extra seconds before resetting.
...
//
// Execute a Obfuscation denomination via a Patriotnode.
// This is only ran from clients
//
void CObfuscationPool::SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fPatriotNode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - Obfuscation from a Patriotnode is not supported currently.\n");
        return;
    }
    ...
    // we should already be connected to a Patriotnode
    if (!sessionFoundPatriotnode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - No Patriotnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Patriotnode updating the progress of Obfuscation
...
bool CObfuscationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    ...
    if (errorID != MSG_NOERR) strAutoDenomResult = _("Patriotnode:") + " " + GetMessageByID(errorID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            ...
            sessionFoundPatriotnode = true;
            //wait for other users. Patriotnode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundPatriotnode) {
            LogPrintf("CObfuscationPool::StatusUpdate - entry not accepted by Patriotnode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Patriotnode
        }
        if (sessionFoundPatriotnode) return true;
    }
...
//
// After we receive the finalized transaction from the Patriotnode, we must
...
bool CObfuscationPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    ...
    // push all of our signatures to the Patriotnode
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (mnodeman.size() == 0) {
        LogPrint("obfuscation", "CObfuscationPool::DoAutomaticDenominating - No Patriotnodes detected\n");
        strAutoDenomResult = _("No Patriotnodes detected.");
        return false;
    }
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeTrumpCoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    // initial phase, find a Patriotnode
    if (!sessionFoundPatriotnode) {
        ...
        //if we've used 90% of the Patriotnode list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(ActiveProtocol()) * 0.9);
        LogPrint("obfuscation", "Checking vecPatriotnodesUsed size %d threshold %d\n", (int)vecPatriotnodesUsed.size(), nThreshold);
        while ((int)vecPatriotnodesUsed.size() > nThreshold) {
            vecPatriotnodesUsed.erase(vecPatriotnodesUsed.begin());
            LogPrint("obfuscation", "  vecPatriotnodesUsed size %d threshold %d\n", (int)vecPatriotnodesUsed.size(), nThreshold);
        }
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                bool fUsed = false;
                //don't reuse Patriotnodes
                BOOST_FOREACH (CTxIn usedVin, vecPatriotnodesUsed) {
                    if (dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                CPatriotnode* pmn = mnodeman.Find(dsq.vin);
                if (pmn == NULL) {
                    LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in patriotnode list!", dsq.vin.ToString());
                    continue;
                }

                LogPrintf("DoAutomaticDenominating --- attempt to connect to patriotnode from queue %s\n", pmn->addr.ToString());
                lastTimeChanged = GetTimeMillis();

                // connect to Patriotnode and submit the queue request
                CNode* pnode = ConnectNode((CAddress)addr, NULL, true);
                if (pnode != NULL) {
                    pSubmittedToPatriotnode = pmn;
                    vecPatriotnodesUsed.push_back(dsq.vin);
                    ...
                    strAutoDenomResult = _("Error connecting to Patriotnode.");
                    dsq.time = 0; //remove node
                    continue;
                }
            }
        ...
        while (i < 10) {
            CPatriotnode* pmn = mnodeman.FindRandomNotInVec(vecPatriotnodesUsed, ActiveProtocol());
            if (pmn == NULL) {
                LogPrintf("DoAutomaticDenominating --- Can't find random patriotnode!\n");
                strAutoDenomResult = _("Can't find random Patriotnode.");
                return false;
            }
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating --- attempt %d connection to Patriotnode %s\n", i, pmn->addr.ToString());
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if (pnode != NULL) {
                pSubmittedToPatriotnode = pmn;
                vecPatriotnodesUsed.push_back(pmn->vin);
                ...
                vecPatriotnodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }

        strAutoDenomResult = _("No compatible Patriotnode found.");
        return false;
    }
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100TRUMP+1 ( bit on if present )
    // bit 1 - 10TRUMP+1
    // bit 2 - 1TRUMP+1
    // bit 3 - .1TRUMP+1
    // bit 3 - non-denom
...
std::string CObfuscationPool::GetMessageByID(int messageID)
{
    ...
    case ERR_MN_LIST:
        return _("Not in the Patriotnode list.");
    case ERR_MODE:
        return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY:
        return _("Non-standard public key detected.");
    case ERR_NOT_A_MN:
        return _("This is not a Patriotnode.");
    case ERR_QUEUE_FULL:
        return _("Patriotnode queue is full.");
    case ERR_RECENT:
...
bool CObfuscationQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strPatriotNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CObfuscationQueue():Relay - ERROR: Invalid Patriotnodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CObfuscationQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyPatriotnode, vchSig, strMessage, errorMessage)) {
            return error("CObfuscationQueue::CheckSignature() - Got bad Patriotnode address signature %s \n", vin.ToString().c_str());
        }
...
void CObfuscationPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    CNode* pnode = FindNode(pSubmittedToPatriotnode->addr);
    if (pnode != NULL) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("trumpcoin-obfuscation");
    ...
    while (true) {
        ...
        if (patriotnodeSync.IsBlockchainSynced()) {
            ...
            if (c % PATRIOTNODE_PING_SECONDS == 1) activePatriotnode.ManageStatus();

            if (c % 60 == 0) {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessPatriotnodeConnections();
                patriotnodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % PATRIOTNODES_DUMP_SECONDS == 0) DumpPatriotnodes();
