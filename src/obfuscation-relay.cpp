...
CObfuScationRelay::CObfuScationRelay(CTxIn& vinPatriotnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinPatriotnode = vinPatriotnodeIn;
    vchSig = vchSigIn;
...
std::string CObfuScationRelay::ToString()
{
    ...
    info << "vin: " << vinPatriotnode.ToString() << " nBlockHeight: " << (int)nBlockHeight << " nRelayType: " << (int)nRelayType << " in " << in.ToString() << " out " << out.ToString();
...
{
    CPatriotnode* pmn = mnodeman.GetPatriotnodeByRank(nRank, nBlockHeight, ActiveProtocol());
