...
{
    if (pszDest == NULL) {
        // we clean patriotnode connections in CPatriotnodeMan::ProcessPatriotnodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActivePatriotnode::ManageStatus()
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "TrumpCoin " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TrumpCoin Core is probably already running."), addrBind.ToString());
        else
...
void RelayInv(CInv& inv)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes){
    		if((pnode->nServices==NODE_BLOOM_WITHOUT_MN) && inv.IsPatriotNodeType())continue;
