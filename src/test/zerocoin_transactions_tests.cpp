...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.ztrumpTracker = unique_ptr<CzTRUMPTracker>(new CzTRUMPTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZTRUMP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZTRUMP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
