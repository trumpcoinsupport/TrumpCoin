...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CBudgetVote>::iterator it1 = mapOrphanPatriotnodeBudgetVotes.begin();
    while (it1 != mapOrphanPatriotnodeBudgetVotes.end()) {
        if (budget.UpdateProposal(((*it1).second), NULL, strError)) {
            ...
            mapOrphanPatriotnodeBudgetVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
CBudgetDB::CBudgetDB()
{
    ...
    strMagicMessage = "PatriotnodeBudget";
}
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 5%% of Patriotnodes: %lli. Number of finalized budgets: %lli\n", 
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 5%% of Patriotnodes: %lli mapFinalizedBudgets.size(): %ld\n", 
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 5% of the network votes, then we should pay a patriotnode instead
    */
    ...
    // If not enough patriotnodes autovoted for any of the finalized budgets pay a patriotnode instead
    if(!fThreshold) {
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnvs") { //Patriotnode vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        Sync(pfrom, nProp);
        LogPrint("mnbudget", "mnvs - Sent Patriotnode votes to peer %i\n", pfrom->GetId());
    }

    if (strCommand == "mprop") { //Patriotnode Proposal
        CBudgetProposalBroadcast budgetProposalBroadcast;
        vRecv >> budgetProposalBroadcast;

        if (mapSeenPatriotnodeBudgetProposals.count(budgetProposalBroadcast.GetHash())) {
            patriotnodeSync.AddedBudgetItem(budgetProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenPatriotnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
    ...
    if (strCommand == "mvote") { //Patriotnode Vote
        CBudgetVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if (mapSeenPatriotnodeBudgetVotes.count(vote.GetHash())) {
            patriotnodeSync.AddedBudgetItem(vote.GetHash());
            return;
        }

        CPatriotnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget","mvote - unknown patriotnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }


        mapSeenPatriotnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
    ...
    if (strCommand == "fbs") { //Finalized Budget Suggestion
        ...
        if (mapSeenFinalizedBudgets.count(finalizedBudgetBroadcast.GetHash())) {
            patriotnodeSync.AddedBudgetItem(finalizedBudgetBroadcast.GetHash());
            return;
        }
    ...
    if (strCommand == "fbvote") { //Finalized Budget Vote
        ...
        CPatriotnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget", "fbvote - unknown patriotnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }
...
void CBudgetManager::ResetSync()
{
    ...
    std::map<uint256, CBudgetProposalBroadcast>::iterator it1 = mapSeenPatriotnodeBudgetProposals.begin();
    while (it1 != mapSeenPatriotnodeBudgetProposals.end()) {
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", PATRIOTNODE_SYNC_BUDGET_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", PATRIOTNODE_SYNC_BUDGET_FIN, nInvCount);
    LogPrint("mnbudget", "CBudgetManager::Sync - sent %d items\n", nInvCount);
}
...
bool CBudgetManager::UpdateProposal(CBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(vote.nProposalHash)) {
        if (pfrom) {
            ...
            mapOrphanPatriotnodeBudgetVotes[vote.nProposalHash] = vote;
...
// If patriotnode voted for a proposal, but is now invalid -- remove the vote
...
bool CBudgetVote::Sign(CKey& keyPatriotnode, CPubKey& pubKeyPatriotnode)
{
    // Choose coins to use
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        if (fDebug){
            LogPrint("mnbudget","CBudgetVote::SignatureValid() - Unknown Patriotnode - %s\n", vin.prevout.hash.ToString());
        }
        return false;
    }
...
//evaluate if we should vote for this. Patriotnode only
void CFinalizedBudget::AutoCheck()
{
    ...
    if (!fPatriotNode || fAutoChecked) {
        LogPrint("mnbudget","CFinalizedBudget::AutoCheck fPatriotNode=%d fAutoChecked=%d\n", fPatriotNode, fAutoChecked);
        return;
    }
...
bool CFinalizedBudget::IsPaidAlready(uint256 nProposalHash, int nBlockHeight)
{
    ...
    // This budget was paid already -> reject transaction so it gets paid to a patriotnode instead
    return true;
}
...
TrxValidationStatus CFinalizedBudget::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    BOOST_FOREACH (CTxOut out, txNew.vout) {
        ...
        if (vecBudgetPayments[nCurrentBudgetPayment].payee == out.scriptPubKey && vecBudgetPayments[nCurrentBudgetPayment].nAmount == out.nValue) {
            ...
            if(paid) {
                LogPrint("mnbudget","CFinalizedBudget::IsTransactionValid - Double Budget Payment of %d for proposal %d detected. Paying a patriotnode instead.\n",
...
void CFinalizedBudget::SubmitVote()
{
    ...
    CFinalizedBudgetVote vote(activePatriotnode.vin, GetHash());
    if (!vote.Sign(keyPatriotnode, pubKeyPatriotnode)) {
...
bool CFinalizedBudgetVote::Sign(CKey& keyPatriotnode, CPubKey& pubKeyPatriotnode)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("mnbudget","CFinalizedBudgetVote::SignatureValid() - Unknown Patriotnode %s\n", strMessage);
        return false;
    }
...
std::string CBudgetManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() << ", Budgets: " << (int)mapFinalizedBudgets.size() << ", Seen Budgets: " << (int)mapSeenPatriotnodeBudgetProposals.size() << ", Seen Budget Votes: " << (int)mapSeenPatriotnodeBudgetVotes.size() << ", Seen Final Budgets: " << (int)mapSeenFinalizedBudgets.size() << ", Seen Final Budget Votes: " << (int)mapSeenFinalizedBudgetVotes.size();
