...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered trumpcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TRUMP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal trumpcoin:URI (trumpcoin:XyZ...?message=example)
            {
...
static void NotifyzTRUMPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTRUMPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTRUMPReset.connect(boost::bind(NotifyzTRUMPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTRUMPReset.disconnect(boost::bind(NotifyzTRUMPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztrumpTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
