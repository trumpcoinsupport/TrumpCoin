                                          ...
                                          banTableContextMenu(0)
{
    ...
    std::string strzTRUMPPathCustom = GetArg("-ztrumpbackuppath", "");
    ...
    if(!strzTRUMPPathCustom.empty()) {
        ui->wallet_customztrumpbackuppath->setText(QString::fromStdString(strzTRUMPPathCustom));
        ui->wallet_customztrumpbackuppath_label->setVisible(true);
        ui->wallet_customztrumpbackuppath->setVisible(true);
    }

    if((!strPathCustom.empty() || !strzTRUMPPathCustom.empty()) && nCustomBackupThreshold > 0) {
...
void RPCConsole::setClientModel(ClientModel* model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setPatriotnodeCount(model->getPatriotnodeCountString());
        connect(model, SIGNAL(strPatriotnodesChanged(QString)), this, SLOT(setPatriotnodeCount(QString)));
...
void RPCConsole::clear()
{
    ...
    message(CMD_REPLY, (tr("Welcome to the TrumpCoin RPC console.") + "<br>" +
...
void RPCConsole::setPatriotnodeCount(const QString& strPatriotnodes)
{
    ui->patriotnodeCount->setText(strPatriotnodes);
}
