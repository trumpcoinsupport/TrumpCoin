...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TRUMP);
    ...
    if (!settings.contains("nAnonymizeTrumpCoinAmount"))
        settings.setValue("nAnonymizeTrumpCoinAmount", 1000);

    nAnonymizeTrumpCoinAmount = settings.value("nAnonymizeTrumpCoinAmount").toLongLong();

    if (!settings.contains("fShowPatriotnodesTab"))
        settings.setValue("fShowPatriotnodesTab", patriotnodeConfig.getCount());
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTrumpCoinAmount"))
        SoftSetArg("-anonymizetrumpcoinamount", settings.value("nAnonymizeTrumpCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in trumpcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTrumpCoinAmount:
            return QVariant(nAnonymizeTrumpCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case SpendZeroConfChange:
            ...
            break;
        case ShowPatriotnodesTab:
            if (settings.value("fShowPatriotnodesTab") != value) {
                settings.setValue("fShowPatriotnodesTab", value);
                setRestartRequired(true);
            }
            break;
        case CoinControlFeatures:
        ...
        case AnonymizeTrumpCoinAmount:
            nAnonymizeTrumpCoinAmount = value.toInt();
