...
#include "qt/trumpcoin/forms/ui_send.h"
#include "qt/trumpcoin/addnewcontactdialog.h"
#include "qt/trumpcoin/qtutils.h"
#include "qt/trumpcoin/sendchangeaddressdialog.h"
...
SendWidget::SendWidget(TrumpCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a TrumpCoin URI"));
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onTRUMPSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onTRUMPSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent TRUMPs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield TRUMPs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent TRUMPs to shield."));
    }
}
...
void SendWidget::onTRUMPSelected(bool _isTransparent)
{
