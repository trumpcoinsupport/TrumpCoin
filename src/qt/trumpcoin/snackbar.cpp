...
#include "qt/trumpcoin/forms/ui_snackbar.h"
#include "qt/trumpcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(TrumpCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &TrumpCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
