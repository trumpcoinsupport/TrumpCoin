...
#include "qt/trumpcoin/forms/ui_mninfodialog.h"
    ...
    ui(new Ui::MnInfoDialog)
{
    ...
    connect(ui->pushCopy, &QPushButton::clicked, [this](){ copyInform(pubKey, tr("Patriotnode public key copied")); });
    connect(ui->pushCopyId, &QPushButton::clicked, [this](){ copyInform(txId, tr("Collateral tx id copied")); });
    connect(ui->pushExport, &QPushButton::clicked, [this](){ exportPN = true; accept(); });
}
