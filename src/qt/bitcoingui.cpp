                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("TrumpCoin Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a TrumpCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and trumpcoin: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zTRUMP"));
    ...
    QSettings settings;
    if (settings.value("fShowPatriotnodesTab").toBool()) {
        patriotnodeAction = new QAction(QIcon(":/icons/patriotnodes"), tr("&Patriotnodes"), this);
        patriotnodeAction->setStatusTip(tr("Browse patriotnodes"));
        patriotnodeAction->setToolTip(patriotnodeAction->statusTip());
        patriotnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        patriotnodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
        patriotnodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
#endif
        tabGroup->addAction(patriotnodeAction);
        connect(patriotnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(patriotnodeAction, SIGNAL(triggered()), this, SLOT(gotoPatriotnodePage()));
    }
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About TrumpCoin Core"), this);
    aboutAction->setStatusTip(tr("Show information about TrumpCoin Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for TrumpCoin"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your TrumpCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TrumpCoin addresses"));
    ...
    openMNConfEditorAction = new QAction(QIcon(":/icons/edit"), tr("Open &Patriotnode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Patriotnode configuration file"));
    ...
    openAction->setStatusTip(tr("Open a TrumpCoin: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the TrumpCoin Core help message to get a list with possible TrumpCoin command-line options"));
...
//        toolbar->widgetForAction(spacer)->setObjectName("ToolbarSpacer");
       ...
       // toolbar->addAction(privacyAction);
        QSettings settings;
        if (settings.value("fShowPatriotnodesTab").toBool()) {
            toolbar->addAction(patriotnodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowPatriotnodesTab").toBool()) {
        patriotnodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("TrumpCoin Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoPatriotnodePage()
{
    QSettings settings;
    if (settings.value("fShowPatriotnodesTab").toBool()) {
        patriotnodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoPatriotnodePage();
    }
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TrumpCoin network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    if (patriotnodeSync.IsBlockchainSynced()) {
        ...
        if (patriotnodeSync.IsSynced()) {
            ...
            nAttempt = patriotnodeSync.RequestedPatriotnodeAttempt < PATRIOTNODE_SYNC_THRESHOLD ?
                           patriotnodeSync.RequestedPatriotnodeAttempt + 1 :
                           PATRIOTNODE_SYNC_THRESHOLD;
            progress = nAttempt + (patriotnodeSync.RequestedPatriotnodeAssets - 1) * PATRIOTNODE_SYNC_THRESHOLD;
            progressBar->setMaximum(4 * PATRIOTNODE_SYNC_THRESHOLD);
            progressBar->setFormat(tr("Synchronizing additional data: %p%"));
            progressBar->setValue(progress);
        }

        strSyncStatus = QString(patriotnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("TrumpCoin Core"); // default title
