...
/** Model for TrumpCoin network client. */
...
public:
    ...
    // Start/Stop the patriotnode polling timer
    void startPatriotnodesTimer();
    void stopPatriotnodesTimer();
    // Force a PN count update calling mnmanager directly locking its internal mutex.
    // Future todo: implement an event based update and remove the lock requirement.
    QString getPatriotnodesCount();

    // Return the specific chain amount value for the PN collateral output.
    CAmount getPNCollateralRequiredAmount();
...
private:
    ...
    QString getPatriotnodeCountString() const;
    OptionsModel* optionsModel;
    PeerTableModel* peerTableModel;
    BanTableModel *banTableModel;

    const CBlockIndex* cacheTip{nullptr};
    QString cachedPatriotnodeCountString;
...
Q_SIGNALS:
    ...
    void strPatriotnodesChanged(const QString& strPatriotnodes);
