...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendPatriotnodeCheckBox->setChecked(pwalletMain->fMultiSendPatriotnodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake && pwalletMain->fMultiSendPatriotnodeReward)
            strMultiSendPrint += "MultiSend Active for Stakes and Patriotnode Rewards\n";
        else if (pwalletMain->fMultiSendStake)
            strMultiSendPrint += "MultiSend Active for Stakes\n";
        else if (pwalletMain->fMultiSendPatriotnodeReward)
            strMultiSendPrint += "MultiSend Active for Patriotnode Rewards\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendPatriotnodeCheckBox->isChecked())) {
        strRet = "Need to select to send on stake and/or patriotnode rewards\n";
        ...
        pwalletMain->fMultiSendPatriotnodeReward = ui->multiSendPatriotnodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendPatriotnodeReward, pwalletMain->nLastMultiSendHeight))
