...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TrumpCoin address (e.g. %1)").arg("C7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
void openConfigfile()
{
    ...
    /* Open trumpcoin.conf with the associated application */
...
{
    boost::filesystem::path pathConfig = GetPatriotnodeConfigFile();

    /* Open patriotnode.conf with the associated application */
    if (boost::filesystem::exists(pathConfig))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathConfig)));
}
 ...
 * the "active-since" list in the patriotnode tab are sorted by the elapsed
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the trumpcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add trumpcoin app to startup item list
