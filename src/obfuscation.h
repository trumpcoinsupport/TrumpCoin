...
#define PATRIOTNODE_ACCEPTED 1
#define PATRIOTNODE_REJECTED 0
#define PATRIOTNODE_RESET -1
...
extern std::string strPatriotNodePrivKey;
...
public:
     ...
     *     1) we have an active Patriotnode,
     *     2) we have a valid Patriotnode private key,
    ...
    /// Check if we have a valid Patriotnode address
    bool CheckSignature();
};
...
{
public:
    /// Is the inputs associated with this public key? (and there is 1000 TRUMP - checking if valid patriotnode)
...
private:
    ...
    std::vector<CObfuScationEntry> entries; // Patriotnode/clients entries
    ...
    bool sessionFoundPatriotnode; //If we've found a compatible Patriotnode
...
public:
    ...
    CPatriotnode* pSubmittedToPatriotnode;
    ...
    {
        if (fPatriotNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            // LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Patriotnode. \n");
            return;
        }
    ...
    /// As a client, send a transaction to a Patriotnode to start the denomination process
    void SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Patriotnode updates about the progress of Obfuscation
    ...
    /// Get the denominations for a specific amount of trumpcoin.
