...
public:
    ...
    public:
        CPatriotnodeEntry(std::string alias, std::string ip, std::string privKey, std::string txHash, std::string outputIndex)
        {
            this->alias = alias;
            this->ip = ip;
    ...
    CPatriotnodeConfig()
    {
        entries = std::vector<CPatriotnodeEntry>();
    }
    ...
    std::vector<CPatriotnodeEntry>& getEntries()
    {
        return entries;
    }
    ...
    {
        int c = -1;
        for (CPatriotnodeEntry e : entries) {
            if (e.getAlias() != "") c++;
        }
        return c;
    }
