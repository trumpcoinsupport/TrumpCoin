...
// The zTRUMP block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zTRUMP is a hash of the serial
...
{
    //Create an output returning the zTRUMP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZTRUMPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTRUMP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTRUMP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTRUMPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTRUMP output", __func__);
...
{
    CzTRUMPTracker* ztrumpTracker = pwallet->ztrumpTracker.get();
    CMintMeta meta;
    if (!ztrumpTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    ztrumpTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a TRUMP stake is the outpoint
