...
static bool CheckZerocoinSpend(const CTransactionRef _tx, CValidationState& state)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZTRUMPModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZTRUMPModule::validateInput(txin, prevOut, tx, ret)){
    ...
    if (consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2)) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zTRUMP spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zTRUMP without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
                                    ...
                                    std::vector<std::pair<CBigNum, uint256>>& vSpendsRet)
{
    for (const CTxIn& txIn : tx.vin) {
        ...
        if (isPublicSpend) {
            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
            PublicCoinSpend publicSpend(params);
            if (!ZTRUMPModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
                return false;
            }
