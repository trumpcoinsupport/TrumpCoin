...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL patriotnodes, open connect to top 1 patriotnode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 patriotnode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == "txlvote") // SwiftX Lock Consensus Votes
    {
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            //Spam/Dos protection
            /*
                Patriotnodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinPatriotnode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinPatriotnode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinPatriotnode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinPatriotnode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - patriotnode is spamming transaction votes: %s %s\n",
                        ctx.vinPatriotnode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which patriotnodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetPatriotnodeRank(activePatriotnode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Unknown Patriotnode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Patriotnode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinPatriotnode = activePatriotnode.vin;
...
{
    int n = mnodeman.GetPatriotnodeRank(ctx.vinPatriotnode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CPatriotnode* pmn = mnodeman.Find(ctx.vinPatriotnode);
    if (pmn != NULL)
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Patriotnode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Unknown Patriotnode\n");
        mnodeman.AskForMN(pnode, ctx.vinPatriotnode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Patriotnode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced patriotnode
        mnodeman.AskForMN(pnode, ctx.vinPatriotnode);
        return false;
    }
...
{
    return vinPatriotnode.prevout.hash + vinPatriotnode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftX::CConsensusVote::SignatureValid() - Unknown Patriotnode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyPatriotnode, vchPatriotNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strPatriotNodePrivKey.c_str());

    if (!obfuScationSigner.SetKey(strPatriotNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid patriotnodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchPatriotNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, vchPatriotNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetPatriotnodeRank(vote.vinPatriotnode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Patriotnode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Patriotnode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
