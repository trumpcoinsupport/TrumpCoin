...
CzTRUMPWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("trumpcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "trumpcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "trumpcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TRUMP and zTRUMP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 15110, 115110));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TRUMP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TRUMP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, trumpcoin, (obfuscation, swiftx, patriotnode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TRUMP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all TrumpCoin specific functionality (Patriotnodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-trumpstake=<n>", strprintf(_("Enable or disable staking functionality for TRUMP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ztrumpstake=<n>", strprintf(_("Enable or disable staking functionality for zTRUMP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageGroup(_("Patriotnode options:"));
    strUsage += HelpMessageOpt("-patriotnode=<n>", strprintf(_("Enable the client to act as a patriotnode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify patriotnode configuration file (default: %s)"), "patriotnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock patriotnodes from patriotnode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-patriotnodeprivkey=<n>", _("Set the patriotnode private key"));
    strUsage += HelpMessageOpt("-patriotnodeaddr=<n>", strprintf(_("Set external address:port to get to this patriotnode (example: %s)"), "128.127.106.235:15110"));
    ...
    strUsage += HelpMessageOpt("-backupztrump=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTRUMP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ztrumpbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zTRUMP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizetrumpcoinamount=<n>     " + strprintf(_("Keep N TRUMP anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 15320, 115320));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The TrumpCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
{
    RenameThread("trumpcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TrumpCoin Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TrumpCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TrumpCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //TrumpCoin: Load Accumulator Checkpoints according to network (main/test/regtest)
        ...
        do {
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZTRUMPMinted();
                        RecalculateZTRUMPSpent();
                    }
                    RecalculateTRUMPSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // TrumpCoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TrumpCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart TrumpCoin Core to complete") << "\n";
        ...
        zwalletMain = new CzTRUMPWallet(pwalletMain->strWalletFile);
   ...
   /*     //Inititalize zTRUMPWallet
    ...
    if (readResult3 == CPatriotnodePaymentDB::FileError)
        ...
        if (readResult3 == CPatriotnodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    fPatriotNode = GetBoolArg("-patriotnode", false);

    if ((fPatriotNode || patriotnodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Patriotnode support requires turning on transaction indexing."
                         "Please add txindex=1 to your configuration and start with -reindex");
    }

    if (fPatriotNode) {
        LogPrintf("IS MASTER NODE\n");
        strPatriotNodeAddr = GetArg("-patriotnodeaddr", "");

        LogPrintf(" addr %s\n", strPatriotNodeAddr.c_str());

        if (!strPatriotNodeAddr.empty()) {
            CService addrTest = CService(strPatriotNodeAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -patriotnodeaddr address: " + strPatriotNodeAddr);
            }
        }

        strPatriotNodePrivKey = GetArg("-patriotnodeprivkey", "");
        if (!strPatriotNodePrivKey.empty()) {
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if (!obfuScationSigner.SetKey(strPatriotNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid patriotnodeprivkey. Please see documenation."));
            }

            activePatriotnode.pubKeyPatriotnode = pubkey;

        } else {
            return InitError(_("You must specify a patriotnodeprivkey in the configuration. Please see documentation for help."));
        }
    }
    ...
    if (GetBoolArg("-mnconflock", true) && pwalletMain) {
        ...
        LogPrintf("Locking Patriotnodes:\n");
        uint256 mnTxHash;
...
//    }
//
//    nAnonymizeTrumpCoinAmount = GetArg("-anonymizetrumpcoinamount", 0);
//    if (nAnonymizeTrumpCoinAmount > 999999) nAnonymizeTrumpCoinAmount = 999999;
//    if (nAnonymizeTrumpCoinAmount < 2) nAnonymizeTrumpCoinAmount = 2;

    fEnableSwiftTX = GetBoolArg("-enableswifttx", fEnableSwiftTX);
    nSwiftTXDepth = GetArg("-swifttxdepth", nSwiftTXDepth);
    nSwiftTXDepth = std::min(std::max(nSwiftTXDepth, 0), 60);

    //lite mode disables all Patriotnode and Obfuscation related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fPatriotNode && fLiteMode) {
        return InitError("You can not start a patriotnode in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nSwiftTXDepth %d\n", nSwiftTXDepth);
    LogPrintf("Anonymize TrumpCoin Amount %d\n", nAnonymizeTrumpCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1TRUMP+1000 == (.1TRUMP+100)*10
       10TRUMP+10000 == (1TRUMP+1000)*10
    */
