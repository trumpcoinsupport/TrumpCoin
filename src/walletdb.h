...
public:
    ...
    bool WriteMSettings(bool fMultiSendStake, bool fMultiSendPatriotnode, int nLastMultiSendHeight);
    ...
    bool WriteZTRUMPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTRUMPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTRUMPSeed_deprecated(uint256& seed);
    bool EraseZTRUMPSeed();
    bool EraseZTRUMPSeed_deprecated();

    bool WriteZTRUMPCount(const uint32_t& nCount);
    bool ReadZTRUMPCount(uint32_t& nCount);
