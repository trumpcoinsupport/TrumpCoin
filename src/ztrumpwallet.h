...
#ifndef TrumpCoin_ZTRUMPWALLET_H
#define TrumpCoin_ZTRUMPWALLET_H
...
public:
    CzTRUMPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTRUMP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTRUMP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TrumpCoin_ZTRUMPWALLET_H
