...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TRUMP at the same time
    ONLY_10000 = 5,                        // find patriotnode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTRUMP send
enum ZerocoinSpendStatus {
    ZTRUMP_SPEND_OKAY = 0,                            // No error
    ZTRUMP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTRUMP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTRUMP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTRUMP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTRUMP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTRUMP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTRUMP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTRUMP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTRUMP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTRUMP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTRUMP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTRUMP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTRUMP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTRUMP_SPENT_USED_ZTRUMP = 14,                      // Coin has already been spend
    ZTRUMP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTRUMP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZTRUMPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fztrumpAuto) const;
    ...
    */
    ...
    CzTRUMPWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzTRUMPTracker> ztrumpTracker;
    ...
    bool fMultiSendPatriotnodeReward;
    ...
    void SetNull()
    {
        ...
        fMultiSendPatriotnodeReward = false;
    ...
    void setZWallet(CzTRUMPWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztrumpTracker = std::unique_ptr<CzTRUMPTracker>(new CzTRUMPTracker(strWalletFile));
    }

    CzTRUMPWallet* getZWallet() { return zwalletMain; }
    ...
    {
        return fMultiSendPatriotnodeReward || fMultiSendStake;
    }
    ...
    /// Get 1000DASH output and keys which can be used for the Patriotnode
    bool GetPatriotnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
    ...
    /** zTRUMP reset */
    boost::signals2::signal<void()> NotifyzTRUMPReset;
