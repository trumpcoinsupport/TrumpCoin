...
CzTRUMPTracker::CzTRUMPTracker(std::string strWalletFile)
{
...
CzTRUMPTracker::~CzTRUMPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTRUMPTracker::Init()
{
...
bool CzTRUMPTracker::Archive(CMintMeta& meta)
{
...
bool CzTRUMPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTRUMPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTRUMPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTRUMPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTRUMPTracker::GetSerialHashes()
{
...
CAmount CzTRUMPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTRUMPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTRUMPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTRUMPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTRUMPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTRUMPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTRUMPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTRUMPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTRUMPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTRUMPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTRUMPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTRUMPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTRUMPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTRUMPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTRUMPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTRUMPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTRUMPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dztrump from DB\n", __func__, listDeterministicDB.size());
    }
